...
public abstract class AbstractEnderLinkCover<T extends VirtualEntry> extends CoverBehavior
    ...
    public void onAttached(@NotNull ItemStack itemStack, @NotNull ServerPlayer player) {
        ...
        if (coverHolder instanceof MachineCoverContainer mcc) {
            try {
                var machineField = MachineCoverContainer.class.getDeclaredField("machine");
                machineField.setAccessible(true);
                this.owner = ((MetaMachine) machineField.get(mcc)).getHolder().getOwner();
            } catch (Exception e) {
                // Ignore
            }
        }
        ...
        private @NotNull ToggleButtonWidget createToggleButton() {
            return (ToggleButtonWidget) new ToggleButtonWidget(0, 0, 12, 12, showChannels::getValue, cd -> {
                ...
                requestUpdate();
                    ...
                    new GuiTextureGroup(GuiTextures.TOGGLE_BUTTON_BACK.getSubTexture(0, 0, 1, 0.5), BUTTON_LIST),
                    new GuiTextureGroup(GuiTextures.TOGGLE_BUTTON_BACK.getSubTexture(0, 0.5, 1, 0.5), BUTTON_LIST))
